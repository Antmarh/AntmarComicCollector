...
try:
    import whakoom_scraper
    WHAKOOM_AVAILABLE = True
except ImportError:
    WHAKOOM_AVAILABLE = False
    print("ADVERTENCIA: No se encontró 'whakoom_scraper.py'. La pestaña de Whakoom estará desactivada.")
...
class MetadataEditorWindow(tk.Toplevel):
    ...
    def apply_whakoom_data(self):
        ...
        if not url or "whakoom.com" not in url.lower():
            messagebox.showwarning("URL Inválida", "Por favor, pega una URL válida de Whakoom.", parent=self); return
        self.status_var_ref.set(f"Obteniendo detalles de Whakoom..."); threading.Thread(target=self._get_whakoom_details_thread, args=(url,), daemon=True).start()
    ...
    def _get_whakoom_details_thread(self, details_url):
        try:
            ...
            self.after(0, self.status_var_ref.set, "Error: No se encuentra 'whakoom_scraper.py'.")
            self.after(0, lambda: messagebox.showerror("Archivo no encontrado", "No se pudo encontrar 'whakoom_scraper.py'.", parent=self))
        except Exception as e:
...
Desarrollado con ❤️ para los amantes del cómic
    ...
    def scan_library_folder(self, sync_mode='full'):
        ...
        try:
            ...
            for file_path in all_files:
                cbz_files.append(str(file_path))
        except Exception as e:
        ...
        if not cbz_files:
            messagebox.showinfo("Finalizado", "No se encontraron archivos .cbz.", parent=self.root)
            self.status_var.set("Listo.")
            self.scan_btn_ref.config(state=tk.NORMAL)
            return
    ...
    def _scan_thread(self, cbz_files, sync_mode, reset_groups=False):
        ...
        try:
            ...
            print(f"Se procesarán {total_a_procesar} cómics nuevos.")
            ...
            for i, cbz_path in enumerate(comics_a_procesar):
                ...
                if reset_groups:
                    # Intentar detectar automáticamente el grupo basándose en metadatos
                    auto_group = self._detect_comic_group(metadata, cbz_path)
                    metadata['Tags'] = auto_group

                data_tuple = (metadata.get('Tags'), metadata.get('Series'), metadata.get('Number'), metadata.get('Title'), metadata.get('Publisher'), metadata.get('Year'), metadata.get('Month'), metadata.get('Day'), metadata.get('Writer'), metadata.get('Penciller'), metadata.get('Inker'), metadata.get('Colorist'), metadata.get('Letterer'), metadata.get('CoverArtist'), metadata.get('Editor'), metadata.get('Summary'), metadata.get('StoryArc'), metadata.get('Characters'), metadata.get('Teams'), metadata.get('Web'), cbz_path)
